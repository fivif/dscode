...
export const useConfigStore = create<ConfigStore>((set, get) => ({
  ...
  saveConfig: async (c) => {
    _pendingConfig = c;
    // Coalesce: resolve all previously pending promises so callers don't hang
  ...
  updateConfig: async (p) => {
    ...
    set({ loading: true });
    try {
      const merged = { ...get().config, ...p };
      set({ config: merged });
      await get().saveConfig(merged);
    } catch (e: any) { console.error('updateConfig failed:', e); }
  ...
  updateProvider: async (provider, p) => {
    ...
    try {
      ...
      const providers = { ...current.providers, [provider]: { ...current.providers[provider], ...p } };
      // If enabling this provider, set it as active
      const active = p.enabled ? provider : current.active_provider;
      const merged = { ...current, providers, active_provider: active };
      set({ config: merged });
      await get().saveConfig(merged);
    } catch (e: any) { console.error('updateProvider failed:', e); }
    finally { set({ loading: false }); }
  },
}));
